...
import hassBioroid from '@/assets/Faction/NISEI_HB.webp';
...
// 卡牌图片动态加载器 — 仅在派系选中时才加载对应的 3 张卡牌
const factionCardLoaders = {
  'HAAS-BIOROID': [
    () => import('@/assets/Faction/HASS/HB.webp'),
    () => import('@/assets/Faction/HASS/liao.webp'),
    () => import('@/assets/Faction/HASS/nike.webp'),
  ],
  'JINTEKI': [
    () => import('@/assets/Faction/JINTEKI/jindan.webp'),
    () => import('@/assets/Faction/JINTEKI/renjian.webp'),
    () => import('@/assets/Faction/JINTEKI/qianma.webp'),
  ],
  'NBN': [
    () => import('@/assets/Faction/NBN/xingyun.webp'),
    () => import('@/assets/Faction/NBN/wangjichuanmei.webp'),
    () => import('@/assets/Faction/NBN/spin.webp'),
  ],
  'WEYLAND': [
    () => import('@/assets/Faction/WEYLAND/tuijin.webp'),
    () => import('@/assets/Faction/WEYLAND/zhengfubutie.webp'),
    () => import('@/assets/Faction/WEYLAND/ziweiji.webp'),
  ],
  'ANARCH': [
    () => import('@/assets/Faction/ANARCH/yelang.webp'),
    () => import('@/assets/Faction/ANARCH/tuopan.webp'),
    () => import('@/assets/Faction/ANARCH/roudu.webp'),
  ],
  'CRIMINAL': [
    () => import('@/assets/Faction/CRIMINAL/sadiya.webp'),
    () => import('@/assets/Faction/CRIMINAL/yingxiao.webp'),
    () => import('@/assets/Faction/CRIMINAL/caifuzhuanyi.webp'),
  ],
  'SHAPER': [
    () => import('@/assets/Faction/SHAPER/tao.webp'),
    () => import('@/assets/Faction/SHAPER/jianzhushi.webp'),
    () => import('@/assets/Faction/SHAPER/chuangzuo.webp'),
  ],
};
...
const FACTION = () => {
  ...
  useEffect(() => {
    if (selectedFaction) {
      const loaders = factionCardLoaders[selectedFaction.subTitle];
      if (loaders) {
      ...
      <div className={styles.rightContent}>
        {selectedFaction && (
          <div className={classNames(styles.contentWrapper, {
            [styles.contentExit]: animationState === 'exit',
            [styles.contentEnter]: animationState === 'enter'
          })}>
            ...
            <div className={styles.textContent}>
              {selectedFaction.content.map((text, index) => (
                <p
                  key={selectedFaction.subTitle + '-text-' + index}
            ...
            <div className={styles.cardContent}>
              {cardImages.map((card, index) => (
                <Image
                  key={selectedFaction.subTitle + '-card-' + index}
                  src={card}
                  ...
                  style={{
                    ...
                    '--reverse-delay': `${(cardImages.length - index - 1) * 0.2}s`
                  }}
                />
              ))}
            </div>
